...
import com.example.freedom.myapplication.Report.Fragment.PiePriority;
            ...
            public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
                ...
                if (items[0].equals(lstTitle.get(groupPosition))) {
                    ...
                    if (selectedItem == "Severity") {  // condition for report
                        Intent intent = new Intent(PriorityActivity.this, Severity.class);
                        ...
                        Intent intent = new Intent(PriorityActivity.this, Typedefect.class);
                        String df_pj_id = getIntent().getStringExtra("df_pj_id");
                        String Username = getIntent().getStringExtra("User_login");
                        String department = getIntent().getStringExtra("Department");
                        ...
                        Intent intent = new Intent(PriorityActivity.this, StatusDefect.class);
                        String df_pj_id = getIntent().getStringExtra("df_pj_id");
                        String Username = getIntent().getStringExtra("User_login");
                        String department = getIntent().getStringExtra("Department");
                    ...
                    if (selectedItem == "Project") {
                        Intent intent = new Intent(PriorityActivity.this, Project_activity.class);
                        String Username = getIntent().getStringExtra("User_login");
                        String department = getIntent().getStringExtra("Department");
                        intent.putExtra("User_login", Username);
                        intent.putExtra("Department", department);
                    ...
                    if (selectedItem == "Defect_log") {
                        Intent intent = new Intent(PriorityActivity.this, Defect_log.class);
                        String df_pj_id = getIntent().getStringExtra("df_pj_id");
                        String Username = getIntent().getStringExtra("User_login");
                        String department = getIntent().getStringExtra("Department");
            ...
            public void run() {
                btnpie.setOnClickListener(new View.OnClickListener() {
                    public void onClick(View v) {
                        ...
                        selectedFragment = PiePriority.newInstance();
//                                PieSeverity pieSeverity = new PieSeverity();
                        Bundle bundlePP = new Bundle();
                        //Pass data Severity
                        String NamePP = getIntent().getStringExtra("NameRe");

                        bundlePP.putString("namere", NamePP);
                        bundlePP.putString("low", String.valueOf(api_reports.get(api_reports.size() - 1).getPrCodeLow()));
                        bundlePP.putString("Medium", String.valueOf(api_reports.get(api_reports.size() - 1).getPrCodeMedium()));
                        bundlePP.putString("High", String.valueOf(api_reports.get(api_reports.size() - 1).getPrCodeHigh()));
                        bundlePP.putString("Urgent", String.valueOf(api_reports.get(api_reports.size() - 1).getPrCodeUrgent()));
                        bundlePP.putString("LowPer", String.valueOf(api_reports.get(api_reports.size() - 1).getPrPercentLow()));
                        bundlePP.putString("MediumPer", String.valueOf(api_reports.get(api_reports.size() - 1).getPrPercentMedium()));
                        bundlePP.putString("HighPer", String.valueOf(api_reports.get(api_reports.size() - 1).getPrPercentHigh()));
                        bundlePP.putString("UrgentPer", String.valueOf(api_reports.get(api_reports.size() - 1).getPrPercentUrgent()));
                        bundlePP.putString("TotalPR", String.valueOf(api_reports.get(api_reports.size() - 1).getPrTotalnumber()));
                        bundlePP.putString("TotalPRPer", String.valueOf(api_reports.get(api_reports.size() - 1).getPrTotalPerNumber()));

//
                        // set Fragmentclass Arguments

                        selectedFragment.setArguments(bundlePP);
